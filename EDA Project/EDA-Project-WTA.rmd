...
3. In the final of tournaments, does total minutes leading to the final have a relationship with win percentage? That is, 
...
# Create winners & losers tables

duplicate_match <- wta %>%
  pivot_longer(c(11,19),
               names_to = "result",
...
# Creating variable that counts # of sets and # of second in
...
How does a player's proportion of break points saved relate to their winning percentage?
...
How does a player's point winning percentage change when they commit a fault on their serve? How does the probability change given they got their second serve in?
...
indiv_player_match %>%
  ...
  summarize_at(vars(df, svpt, first_in, first_won, second_in, second_won), sum, 
               na.rm = TRUE) %>%
  ungroup() %>%
...
In the final of tournaments, does total minutes leading to the final have a relationship with win percentage?
...
```{r}
# Clustering: Observing player aggression using aces and df
```
